...
public class GT_Loader_OreDictionary extends gregtech.loaders.preload.LoaderGTOreDictionary implements Runnable {
    ...
    // (see IngredientFactory.invalidItem). Registering that placeholder claims real ore-dictionary slots for
    // one and the same stack, which then feeds GT's autogenerated processing recipes. Skip those
    // registrations; with all mods present these guards never trigger.
