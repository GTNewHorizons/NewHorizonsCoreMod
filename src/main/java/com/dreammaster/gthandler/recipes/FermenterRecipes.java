...
public class FermenterRecipes implements Runnable {
    ...
    public void run() {
        ...
        GTValues.RA.stdBuilder().fluidInputs(
                MaterialLibAPI
                        .getFluidStack(Materials2Materials.Concrete, Materials2FluidShapes.fluidMolten, (int) (1000)))
                .fluidOutputs(MU.materialOf(Materials2Materials.ConstructionFoam).getFluid(1000)).duration(1 * SECONDS)
                .eut(TierEU.RECIPE_LV / 2).addTo(fermentingRecipes);

        GTValues.RA.stdBuilder().fluidInputs(MU.materialOf(Materials2Materials.ConstructionFoam).getFluid(1000))
