...
public class FluidExtractorRecipes implements Runnable {
    ...
    public void run() {
        ...
        GT_Values.RA.stdBuilder().itemInputs(new ItemStack(Items.glass_bottle, 1, 0)).itemOutputs(GT_Values.NI)
                .noFluidInputs().fluidOutputs(Materials.Glass.getMolten(144L)).duration(1 * SECONDS + 4 * TICKS).eut(54)
                .addTo(sFluidExtractionRecipes);

        GT_Values.RA.stdBuilder().itemInputs(ItemList.GalliumArsenideCrystal.get(1L)).itemOutputs(GT_Values.NI)
                .noFluidInputs().fluidOutputs(Materials.GalliumArsenide.getMolten(288L))
                .duration(2 * SECONDS + 8 * TICKS).eut(37).addTo(sFluidExtractionRecipes);

        if (PamsHarvestCraft.isModLoaded()) {

            GT_Values.RA.stdBuilder().itemInputs(GT_ModHandler.getModItem(PamsHarvestCraft.ID, "catfishrawItem", 1, 0))
                    .itemOutputs(GT_Values.NI).noFluidInputs().fluidOutputs(Materials.FishOil.getFluid(50L))
            ...
            GT_Values.RA.stdBuilder()
                    .itemInputs(GT_ModHandler.getModItem(PamsHarvestCraft.ID, "jellyfishrawItem", 1, 0))
                    .itemOutputs(GT_Values.NI).noFluidInputs().fluidOutputs(Materials.FishOil.getFluid(50L))
                    .duration(16 * TICKS).eut(4).addTo(sFluidExtractionRecipes);
        ...
        if (OpenBlocks.isModLoaded() && TinkerConstruct.isModLoaded()) {
            ...
            GT_Values.RA.stdBuilder().itemInputs(GT_ModHandler.getModItem(TinkerConstruct.ID, "oreBerries", 1L, 5))
                    .itemOutputs(GT_Values.NI).noFluidInputs()
        ...
        if (TinkerConstruct.isModLoaded()) {
            ...
            GT_Values.RA.stdBuilder().itemInputs(new ItemStack(Items.ender_pearl, 1, 0)).itemOutputs(GT_Values.NI)
                    .noFluidInputs().fluidOutputs(new FluidStack(FluidRegistry.getFluid("ender"), 250))
                    .duration(5 * SECONDS).eut(TierEU.RECIPE_LV).addTo(sFluidExtractionRecipes);
            ...
            GT_Values.RA.stdBuilder().itemInputs(new ItemStack(Items.slime_ball, 1, 0)).itemOutputs(GT_Values.NI)
                    .noFluidInputs().fluidOutputs(new FluidStack(FluidRegistry.getFluid("glue"), 144))
