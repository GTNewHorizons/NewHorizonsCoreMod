...
public class WitcheryPlugin extends BasePluginWitchery {
    ...
    private static void addBrewRecipe(int power, ItemStack result, ItemStack lastItem, ItemStack... items) {
        ...
        if (action == null) {
            registerBrewAction(new BrewActionRitualRecipe(
                    key, new AltarPower(power), new BrewActionRitualRecipe.Recipe(result, items)));
            ...
            for (BrewActionRitualRecipe.Recipe recipe : ritualRecipe.getExpandedRecipes()) {
                if (isCauldronRecipeMatch(recipe, lastItem, items)) {
                    if (ritualRecipe.getExpandedRecipes().size() > 1) {
                        ...
                        modifyBrewRecipe(
                                ritualRecipe,
                                ritualRecipe.getExpandedRecipes().stream()
                                        .filter(r -> r != recipe)
                                        .map(r -> new BrewActionRitualRecipe.Recipe(
                                                r.result, Arrays.copyOf(r.ingredients, r.ingredients.length - 1)))
    ...
    public boolean init() {
        ...
        SpinningWheel.addRecipe(
                ...
                new ItemStack[] {
                    new ItemStack(Items.string, 8, 0), Witchery.Items.GENERIC.itemWhiffOfMagic.createStack(1)
                });
        SpinningWheel.addRecipe(
                Witchery.Items.GENERIC.itemGoldenThread.createStack(1),
                Witchery.Items.GENERIC.itemFancifulThread.createStack(1),
        ...
        SpinningWheel.addRecipe(new ItemStack(Items.string, 8, 0), new ItemStack(Blocks.web, 1, 0), new ItemStack[] {
            Witchery.Items.GENERIC.itemWhiffOfMagic.createStack(1)
        });
